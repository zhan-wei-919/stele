//! Native desktop runner for the bundled Stele demo.
...
use crate::io::{IoRuntime, UiEffectDriver, ViewUpdateDriver, WakeEvent};
use crate::renderer::Renderer;
use crate::scene::SceneConfig;
use crate::store::{run_store, ViewportState};

const WINDOW_WIDTH: f64 = 960.0;
const WINDOW_HEIGHT: f64 = 640.0;

/// Runs the bundled desktop demo using Stele's native winit/wgpu shell.
pub fn run_demo_app() -> Result<(), Box<dyn Error>> {
    let scene_config =
    ...
    app.install_store_bootstrap(Box::new(move |runtime: &IoRuntime, size, scale_factor| {
        ...
        let store = crate::demo::build_store(
            rasterizer,
...
fn build_rasterizer() -> FreeTypeRasterizer {
    ...
    let subpixel_layout = crate::renderer::subpixel::detect_subpixel_layout();
