//! Internal helpers for allocating typed GPU buffers used by the renderer runtime.
...
/// Allocates an index buffer with enough capacity for at least one element.
pub(super) fn create_index_buffer<T>(
    device: &wgpu::Device,
    capacity: usize,
    label: &str,
) -> wgpu::Buffer {
...
/// Grows an index buffer only when the current capacity is insufficient.
pub(super) fn ensure_index_capacity<T>(
    device: &wgpu::Device,
    required: usize,
    buffer: &mut wgpu::Buffer,
    capacity: &mut usize,
    ...
    *capacity = required.max(1).next_power_of_two();
    *buffer = create_index_buffer::<T>(device, *capacity, label);
}
